...
const SearchResults = styled.div.attrs(({ searchActive }: { searchActive: boolean }): any => ({
...
const filterPaths = (item: SwagoMethod, filter: (item: SwagoMethod) => boolean, matches: SwagoMethod[]) => {
  if (filter(item)) {
    matches.push(item);
  }
...
const hasOpsFilter: PathFilter = (item: SwagoMethod) => (item ? item.operations.filter(o => o.operation).length > 0 : false);
...
export const MethodExplorer =
  (props: Props): JSX.Element => {
    ...
    const [selectedSearchItem, setSelectedSearchItem] = React.useState<SwagoMethod>();
    ...
    const renderSearchItem = (itemProps: RenderListItemProps<SwagoMethod>) => {
      return <MethodListItem method={itemProps.item} selected={itemProps.selected} />;
    };
    ...
    return (
      <Root>
        ...
        <LeftRightSplit initialLeftGridWidth={'35%'} minLeftPixels={0} minRightPixels={0}>
          <LeftPane>
            <Navigation>
              <CommandBar>
                ...
                {methods && (
                  <>
                    <ExpandCollapseButton title='collapse all' onClick={() => setTreeExpanded(false)}>
                      <FontAwesomeIcon icon={faCompressAlt} />
                    </ExpandCollapseButton>
                    <ExpandCollapseButton title='expand all' onClick={() => setTreeExpanded(true)}>
          ...
          <RightPane>
            <PathDetails>{selectedDetails && <MethodCard method={selectedDetails} />}</PathDetails>
          </RightPane>
        </LeftRightSplit>
      </Root>
    );
