...
export const TypeDefinitionExplorer = (props: Props): JSX.Element => {
  ...
  const renderListItem = (itemProps: RenderListItemProps<SwagoTypeDefinition>) => {
    return <TypeDefinitionListItem typeDefinition={itemProps.item} selected={itemProps.selected} />;
  };
    ...
    <Root>
      ...
      <LeftRightSplit initialLeftGridWidth={'35%'} minLeftPixels={0} minRightPixels={0}>
        <Navigation>
          ...
          <List getKey={getKey} items={items} renderItem={renderListItem} selectedItem={selectedTypeDefinition} onSelected={onSelected} />
        </Navigation>
        <TypeDetails>{selectedTypeDefinition && <TypeDefinitionCard typeDefinition={selectedTypeDefinition} />}</TypeDetails>
      </LeftRightSplit>
    </Root>
  );
};
